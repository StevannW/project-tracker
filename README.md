...
### 1. Clone Repository
Download source code project ini. Buka terminal di device, lalu ketik command:
```bash
git clone https://github.com/StevannW/project-tracker.git
```
Setelah selesai download, masuk ke directory project dengan command:
...
Copy file kredensial contoh yaitu `.env.example` menjadi file `.env` resmi. Jalankan perintah ini di terminal:
...
Setelah file `.env` berhasil di copy, jalankan perintah `docker compose`:
```bash
...
Tekan `Ctrl+C` pada terminal yang sedang berjalan. Jika ingin mematikan *container* yang berjalan di background, gunakan:
```bash
docker compose down
```
*(Opsional)* Jika ingin mereset/menghapus seluruh isi database task yang tersimpan:
```bash
...
Untuk memastikan aplikasi tersebut sudah terinstal dengan benar beserta versinya, jalankan perintah berikut di terminal:
```bash
psql -V
python --version
node -v
...
Pastikan layanan database PostgreSQL sudah berjalan. Buat database baru dengan nama `task_tracker` menggunakan salah satu dari dua cara berikut:

**Cara A: Menggunakan CLI (psql)**
Buka terminal atau Command Prompt, lalu ikuti langkah berikut:
1. Masuk ke PostgreSQL menggunakan *user* default (`postgres`):
   ```bash
   psql -U postgres
   ```
2. Masukkan *password* yang dibuat saat pertama kali menginstal PostgreSQL.
...
1. Buka aplikasi **pgAdmin 4** di komputer dan login menggunakan *password* yang dibuat saat pertama kali menginstal PostgreSQL.
2. Di panel sebelah kiri, perluas (*expand*) menu **Servers** > **PostgreSQL 16** (atau versi yang digunakan).
...
cd backend
```
Buat (*Virtual Environment*) agar instalasi library terisolasi, jalankan command ini:
```bash
python -m venv .venv
```
